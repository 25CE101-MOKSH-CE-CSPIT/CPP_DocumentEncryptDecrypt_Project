...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            int result = Encryptor::decryptFile(inputPath, outputPath, password);
            if (result == Encryptor::kDecryptOk) {
                ...
                std::cout << RED << "Error: File is not a valid encrypted file!" << RESET << "\n";
                ...
                std::cout << RED << "Error: File data is corrupted!" << RESET << "\n";
            } else {
                std::cout << RED << "Error: Decryption Failed!" << RESET << "\n";
            }
        }
