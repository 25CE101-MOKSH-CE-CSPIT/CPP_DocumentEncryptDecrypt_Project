...
void ExecuteAction(HWND hwnd, bool encrypt) {
    ...
    if (encrypt) {
        ...
        int result = Encryptor::decryptFile(iS, o, pS);
        if (result == Encryptor::kDecryptOk) { 
            ...
            statusColor = 1; SetWindowText(hStatus, "Error : File is not a valid encrypted file !");
        } else if (result == Encryptor::kDecryptWrongPassword) {
            statusColor = 1; SetWindowText(hStatus, "Error : Wrong Password !");
        } else if (result == Encryptor::kDecryptDataError) {
            statusColor = 1; SetWindowText(hStatus, "Error : File data is corrupted !");
        } else {
            statusColor = 1; SetWindowText(hStatus, "Error : Decryption Failed !");
        }
    }
